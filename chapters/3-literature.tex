...
The author represented a model with an image encoder, text encoder, and a multimodal encoder. They use 12-layer visual transformer ViT-B/16[\cite{dosovitskiy2021image}], pretrained with ImageNet-1k, as the image encoder. Text encoder is based on 6 layers of transformer, and 6 layers of transformer for multimodal encoder. Those two encoders are pretrained with first 6 layers of $BERT_{base}$ for text encoders and last 6 layers of $BERT_{base}$ for multimodal encoders. The image features are fused with text features with cross attention in multimodal encoders.

Pretraining Objectives
The model have three objectives in pretraining: Image-Text Contrastive learining, masked language modeling and image text matchng.
Image text contrastive learning is used to align the representation from the unimodal encoders. This task is done to align the represenation before they are fused.

Masked languag modeling (MLM) is utilized with both image and text information to determine the masked text. Author randomly masks 15\% of the input token with specialized token [MASK]. The MLM minimizes the cross-entropy loss.
\begin{displaymath}
...
Contrastive loss is used for alignment, which helps to ground the vision and text represenations.
