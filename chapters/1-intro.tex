...
% This thesis explores the attention differences between text and image information. Many approaches have been developed to achieve better multi-modal representation, but they often assume that the masking ratio in Vision-Language Models (VLMs) does not significantly impact model performance. However, we hypothesize that the masking ratio plays a crucial role in balancing the attention between text and image modalities, potentially affecting the accuracy of person retrieval models. In this work, we challenge this assumption by investigating the role of the masking ratio in person retrieval models. Special attention is given to how different masking ratios influence model accuracy. We analyze these influences to determine if and how the accuracy changes.


% This thesis explores the attention differences between text and image information. Many approaches have been developed to achieve better multi-modal representation, but they often assume that the masking ratio in Vision-Language Models (VLMs) does not significantly impact model performance. In this work, we challenge this assumption by investigating the role of the masking ratio in person retrieval models. Special attention is given to how different masking ratios influence model accuracy. We analyze these influences to determine if and how the accuracy changes.

% This thesis is tackling through the attention differences between text and image information. Many approaches have been developed to achieve better multi-modal representation, but they lack an investigation of the masking ratio when it comes to VLM models. In this work, we tackle the attention differences between text and image information applied to person retrieval models. Special attention is given to the influences of the masking ratio on the model accuracy. We look into this and see if the accuracy changes.
